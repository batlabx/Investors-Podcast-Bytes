...
def existing_episodes(repo_dir: Path) -> tuple[set[str], int]:
    ...
    Transcripts now live under ``Transcripts/``. Older repos may still have
    files at root, so we scan both for backwards compatibility.
    """
    seen = set()
    high = 0
    transcripts_dir = repo_dir / "Transcripts"
    ...
    patterns = [
        str(transcripts_dir / "[0-9]*_*.txt"),
        str(repo_dir / "[0-9]*_*.txt"),  # legacy fallback
    ]
...
def regenerate_readme(repo_dir: Path) -> Path:
    """Regenerate the auto-generated index of episodes.

    Writes ``Transcripts/INDEX.md`` (and never touches ``README.md`` — that
    file is curated by hand and contains the Substack subscribe pitch).
    """
    transcripts_dir = repo_dir / "Transcripts"
    transcripts_dir.mkdir(exist_ok=True)
    files = sorted(glob.glob(str(transcripts_dir / "[0-9]*_*.txt")),
    ...
    out = [
        "# Episode Index",
        "",
        f"Auto-generated index of every episode summary in this folder. **{len(rows)} episodes** archived, most recent first.",
        "",
        "Want the daily 5-minute read instead of digging through these `.txt` files? "
        "**[Subscribe on Substack →](https://batlab.substack.com)**",
        "",
        "| # | Title | Date | Source |",
        "|---|-------|------|--------|",
    ]
...
def main() -> int:
    ...
    print(f"Wrote {written} new files. Regenerating Transcripts/INDEX.md…")
    regenerate_readme(REPO_DIR)
